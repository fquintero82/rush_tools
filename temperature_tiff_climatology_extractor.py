...
class TemperatureTiffExtractor:
    ...
    def get_file_path(self, doy):
        """
        Constructs the filename: mean_DOY.tif
        Example: mean_1.tif
        """
        filename = f"mean_{doy}.tif"
        return os.path.join(self.folder_path, filename)

    def extract_period(self, start_unixtime, end_unixtime):
...
if __name__=="__main__":
    # --- Usage Example ---
    folder = "V:\\data\\era5\\temperature\\daily_climatology\\"
    lats = np.random.uniform(40, 46, 1000000)
    lons = np.random.uniform(-89, -98, 1000000)
    extractor = TemperatureTiffExtractor(folder, lats, lons)
    # # Get full year 2002
    tmp = extractor.extract_period( 1783987200, 1784073600)
    print(tmp.shape)  # Should be (1000000, 10)
    print('here')