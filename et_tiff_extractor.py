...
class FolderTiffExtractor:
    ...
    def get_file_path(self, year, doy):
        """
        Constructs the filename: detYYYYDOY.modisSSEBopETactual.tif
        Example: det2000001.modisSSEBopETactual.tif
        """
        filename = f"det{year}{doy:03d}.modisSSEBopETactual.tif"
        return os.path.join(self.folder_path, filename)

    def extract_period(self, year, start_doy, end_doy):
        """
        ...
        for i, doy in enumerate(range(start_doy, end_doy + 1)):
            file_path = self.get_file_path(year, doy)
...
if __name__=="__main__":
    # --- Usage Example ---
    et_folder = "/home/fquinteroduque/LSS/preclab/modisSSEBop/2002"
    et_folder = "V:\\modisSSEBop\\2002"
    lats = np.random.uniform(25, 49, 1000000)
    lons = np.random.uniform(-125, -67, 1000000)
    extractor = FolderTiffExtractor(et_folder, lats, lons)
    # # Get full year 2002
    et_2002 = extractor.extract_period(2002, 1, 1)
    print('here')